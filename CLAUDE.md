# Claude Code Instructions for WOTD

This file guides Claude through WOTD development, from planning through refinement. Read this before asking Claude to help with any aspect of the project.

---

## Project Context

**Project**: WOTD - An Obsidian plugin  
**Tech Stack**: TypeScript, ESLint (obsidianmd plugin), Vitest (unit), Playwright (E2E), npm
**Build Output**: `vault/.obsidian/plugins/WOTD` (single vault for both dev and E2E testing)
...
# 3. Build the plugin
npm run build
# This compiles TypeScript and outputs to vault/.obsidian/plugins/WOTD
...
**Output Location**: `vault/.obsidian/plugins/WOTD/` (single vault for dev and E2E testing)
...
### File Operations (if WOTD handles files)
...
### Plugin Settings (if WOTD has settings)

If WOTD persists settings:
- Load settings on plugin load
- Validate settings when loading
- Provide sensible defaults
- Notify user of changes

### Event Listeners (if WOTD listens to vault events)
...
WOTD runs in Obsidian, which is resource-constrained on some systems.
...
- Reload the plugin to test changes: Community Plugins → WOTD → Reload
...
**For Claude**: This file is your guide. Reference it before helping with any WOTD code.
